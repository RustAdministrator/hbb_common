...
impl UserDefaultConfig {
    ...
    pub fn get(&self, key: &str) -> String {
        match key {
            ...
            keys::OPTION_VIDEO_PROFILE => self.get_string(key, "standard", vec!["movie"]),
...
mod tests {
    ...
    #[test]
    fn test_video_profile_defaults_and_normalizes() {
        let mut cfg = UserDefaultConfig::default();
        assert_eq!(cfg.get(keys::OPTION_VIDEO_PROFILE), "standard");

        cfg.options
            .insert(keys::OPTION_VIDEO_PROFILE.to_owned(), "movie".to_owned());
        assert_eq!(cfg.get(keys::OPTION_VIDEO_PROFILE), "movie");
